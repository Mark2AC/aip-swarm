...
3. **네임스페이스 규약** — 새 토픽은 `/<ns>/` 접두사 준수 (`aip1`, `aip2`, `aip3`)
4. **시크릿 없음** — `secrets.ini`, `.env`, `keystore/` 포함 금지
5. **차량 온보드 SW 불변** — 차량 자체 SW는 담당 팀원 관할, 본 리포에서 수정 금지
...
```bash
git clone --recurse-submodules https://github.com/Mark2AC/aip-swarm.git ~/aip_swarm_ws
bash ~/aip_swarm_ws/scripts/setup_ubuntu.sh
source ~/aip_swarm_ws/aip_env.sh
aip_build
```

자세한 내용: [`docs/USAGE.md`](docs/USAGE.md)

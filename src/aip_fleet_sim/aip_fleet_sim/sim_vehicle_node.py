...
class SimVehicleNode(Node):
    ...
    def _publish_heartbeat(self) -> None:
        m = FleetHeartbeat()
        m.vehicle_id = self.vid
        m.stamp = self.get_clock().now().to_msg()
        if self._estop:
            m.state = FleetHeartbeat.STATE_ESTOP
        elif abs(self._cmd_v) + abs(self._cmd_w) > 1e-3:
            m.state = FleetHeartbeat.STATE_AUTO
        else:
            m.state = FleetHeartbeat.STATE_IDLE
        m.battery_pct = self._battery
        m.cpu_load = 0.1
        m.active_behaviors = ['sim']
